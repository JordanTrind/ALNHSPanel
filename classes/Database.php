<?php
    class Database {
        ...
        function __construct($object = NULL) {
            if(!self::$conn) {
                $mysqli = @new mysqli("localhost", "root", "", "nhspanel");

                if (@$mysqli->connect_errno) {
                    Msg("Cant connect to DB!", $this->variable_errorcode);
                }

                if (!@$mysqli->set_charset("utf8")) {
                    Msg("Cant set DB charset!", $this->variable_errorcode + 7);
                }

                if (!@$mysqli->query("SET time_zone = '+00:00'")) {
                    Msg("Cant set DB timezone!", $this->variable_errorcode + 8);
                }

                self::$conn = $mysqli;
            }

            if (is_object($object) && tableName($this) != "Database") {
        ...
        public function insert($values = NULL, $log = NULL, $reason = "Unknown", $table_name = NULL) {
            ...
            $table_name = (is_null($table_name)) ? tableName($this) : tableName($table_name);
        ...
        public function getFromDB($value = NULL, $colum = NULL, $return = "auto", $limit_start = false, $limit_end = false, $order = false, $desc = false, $operator = "=", $select = NULL, $case = true, $table_name = NULL) {
            $table_name = ($table_name === NULL) ? tableName($this) : tableName($table_name);
            ...
            if ($return === "auto") {
                if ($value === false) {
                    throw new Exception('Value can not be empty when using "auto" as return', $this->variable_errorcode);
                }
        ...
        public function runSQL($sql, $return = "result", $log = true, $reason = "Unknown") {
            if ($result = self::$conn->query($sql)) {
                ...
                throw new Exception("Failed to run SQL!", $this->variable_errorcode + 6);
            }
        }

        private function _insert($table_name, $fields_and_values, $log = true, $reason = "Unknown") {
            ...
            if (self::$conn->query($sql)) {
                ...
                throw new Exception("Failed to insert to DB!", $this->variable_errorcode + 1);
            }
        }
        ...
        private function _update($table_name, $to_be_updated, $record_identifyer_value, $record_identifyer_name, $log, $reason) {
            ...
            if (count($to_be_updated)) {
                ...
                if ($result = self::$conn->query($sql)) {
                    ...
                    throw new Exception("Failed to update in DB!", $this->variable_errorcode + 2);
                }
            }
        }
        ...
        private function _delete($table_name, $record_identifyer_value, $record_identifyer_name, $log, $reason) {
            ...
            if ($result = self::$conn->query($sql)) {
                ...
                throw new Exception("Failed to delete from DB!", $this->variable_errorcode + 3);
            }
        }
        ...
        private function _getFromDB($table_name, $colum, $value, $return, $limit_start, $limit_end, $order, $desc, $operator, $case, $select) {
            ...
            if ($result = self::$conn->query($sql)) {
                ...
                throw new Exception("Failed to select in DB!", $this->variable_errorcode + 5);
            }
        }

        public function setupDatabase($tablename, $tabledata, $defaultrows, $log = true) {
            return true;
